    ...
    <div class="content-wrapper">
        ...
        <div class="card">
            <div class="card-body">
                ...
                <form id="edit-form" action="{{ route('admin.videos.update', $video->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <input type="hidden" name="course_id" value="{{ $course->id }}">
                    <input type="hidden" id="filename" name="filename" value="{{ $video->filename }}" />
                    ...
                    <div class="row gy-20">
                        ...
                        <div class="col-sm-12 mt-3">
                            ...
                            <textarea name="description" class="form-control @error('description') is-invalid @enderror" 
                                      required rows="4">{{ old('description', $video->description) }}</textarea>
                        ...
                        <div class="col-sm-6 mt-3">
                            ...
                            <select name="type" id="uploadType" class="form-select" required>
                                ...
                                <option value="url" {{ old('type', $video->type) == 'url' ? 'selected' : '' }}>URL</option>
                        ...
                        <div class="col-sm-6 mt-3" id="storageSection" style="display: none;">
                            ...
                            <select name="storage" id="storage" class="form-select">
                                <option value="">-- Pilih</option>
                                <option value="youtube" {{ old('storage', $video->storage) == 'youtube' ? 'selected' : '' }}>Upload YouTube</option>
                                <option value="aws" {{ old('storage', $video->storage) == 'aws' ? 'selected' : '' }}>Storage AWS</option>
                            </select>
                        </div>
                        ...
                        <div class="col-sm-12 mt-3" id="urlSection" style="display: none;">
                            ...
                            <input type="text" name="link_url" id="link_url" 
                                   ...
                                   value="{{ old('link_url', $video->link_url) }}">
                        ...
                        <div class="col-sm-12 mt-3" id="videoSection" style="display: none;">
                            <div class="card border">
                                ...
                                <div class="card-footer p-4" style="display: {{ $video->type == 'video' ? 'block' : 'none' }};">
                                    <video id="videoPreview" controls style="width: 100%; height: auto;"
                                           src="{{ $video->storage == 'aws' ? Storage::disk('s3')->url($video->filename) : '' }}">
                                    </video>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="col-sm-12 mt-4">
                            ...
                            <button type="submit" class="btn btn-primary">
                                <i class="ph ph-cloud-arrow-up me-2"></i> Update Video
                            </button>
                        </div>
                    </div>
                </form>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function uploadFileToS3(file, onProgress) {
        ...
        if (!response.ok) throw new Error('Gagal mendapatkan presigned URL');
