                    ...
                    <div class="navbar-wrapper">
                        ...
                        <nav role="navigation" class="nav-menu-wrapper w-nav-menu">
                            <ul role="list" class="nav-menu-two w-list-unstyled">
                                ...
                                @auth
                                    @php
                                        $latestOtp = Auth::user()->otps()->latest()->first();
                                        $otpPending = $latestOtp && $latestOtp->status === 'pending';
                                    @endphp

                                    @if (!$otpPending)
                                        <li>
                                            <div data-hover="false" data-delay="0" class="nav-dropdown w-dropdown">
                                                <div class="nav-dropdown-toggle w-dropdown-toggle button-primary w-button"
                                                ...
                                                <nav class="nav-dropdown-list shadow-three w-dropdown-list">
                                                    @if (Auth::user()->role === 'admin')
                                                        <a href="{{ route('admin.dashboard') }}"
                                                            class="nav-dropdown-link w-dropdown-link">CMS</a>
                                                    ...
                                                    <a href="{{ route('troopers.my-course') }}"
                                                        class="nav-dropdown-link w-dropdown-link">My Classes</a>
                                    ...
                                    @else
                                        <li class="mobile-margin-top-10">
                                            <a href="{{ route('login-first') }}" class="button-primary w-button">Login</a>
                                        </li>
                                    @endif
                                @endauth
